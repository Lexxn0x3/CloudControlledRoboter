...
class RobotController:
    def __init__(self, bot_instance ,model_path='/opt/roboterteam1/appcontroller/appcontroller/detect/werner.pt', ip_address='127.0.0.1', port=9001):
    ...
    def adjust_robot_position(self, object_midpoint, target_position, tolerance):
        ...
        if object_x >= abs(target_x - tolerance_x)  and object_x <= abs(target_x + tolerance_x) and final_x:
            self.bot.stop()
            print("stop")
            final_x = True
        ...
        if object_y >= abs(target_y - tolerance_y)  and object_y <= abs(target_y + tolerance_y) and final_x and not final_y:
            self.bot.stop()
            print("stop")
            final_y = True

        if object_y < abs(target_y - tolerance_y)  or object_y > abs(target_y + tolerance_y) and final_x:
