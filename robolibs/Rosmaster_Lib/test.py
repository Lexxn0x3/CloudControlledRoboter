...
while True:
    ...
    try:
        ...
        com = 'COM%d' % com_index
        g_bot = Rosmaster(1, com, debug=g_debug)
        break
    except:
        if com_index > 256:
            print("-----------------------No COM Open--------------------------")
...
def parse_data(sk_client, data):
    ...
    # 控制阿克曼小车前轮舵机角度
        ...
        if -45 <= num_angle <= 45 and num_id == AKM_PWMSERVO_ID:
            g_bot.set_akm_steering_angle(num_angle, g_car_stabilize_state)
...
if __name__ == '__main__':
    ...
    print("Car Type:", g_bot.get_car_type_from_machine())
    print("Waiting for connect to the APP!")

    try:
        init_tcp_socket('192.168.2.71')
        while True:
            pass
            time.sleep(10)
